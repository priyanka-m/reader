<div id = "controls">
	...
	<!-- Set dir value to be passed to the showPDFviewer function -->
...
<?php
	...
	/* Display pdf files in the current directory and links to sub-directories 
	* inside the current directory which contain pdf files. */
?>
	...
	<tbody id = "fileList">
		...
		// Array to store directory entries, which contain pdfs.
			...
			foreach ($files as $file) {
				// Encode the file and directory names.
		...
		?>			<!-- Each tr displays a file -->	
					<tr data-file="<?php echo $name;?>" data-type="<?php echo 'file'?>" data-mime="<?php echo 'application/pdf'?>">
						<td class="filename svg" style="background-image:url(<?php echo OCP\mimetype_icon('application/pdf'); ?>)">
							<!-- link passes to files/download.php that renders contents of the file -->
							<a class="name" href="<?php echo \OCP\Util::linkTo('files', 'download.php').'?file='.$directory.$name; ?>" title="">
								<span class = "nametext">
		...
		<?php
					...
					$d = '<tr data-file="'.$file['filename'].'" data-type="dir" data-mime="dir">
